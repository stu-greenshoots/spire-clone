 ...
 * Uses WebP format (optimized) with PNG fallback handled by preloadEnemyImage.
...
/**
 * Get the PNG fallback path for an enemy ID.
 * @param {string} enemyId - The enemy identifier
 * @returns {string} The PNG fallback path
 */
export function getEnemyImagePathPng(enemyId) {
...
export function hasImage(enemyId) {
  const webpPath = getEnemyImagePath(enemyId);
  const webpCached = imageCache.get(webpPath);
  if (webpCached && webpCached.loaded) return true;
  const pngPath = getEnemyImagePathPng(enemyId);
  const pngCached = imageCache.get(pngPath);
  return pngCached ? pngCached.loaded : false;
}

/**
 * Preload the image for a specific enemy by ID.
 * Tries WebP first, falls back to PNG if WebP fails.
 * @param {string} enemyId - The enemy identifier
...
export function preloadEnemyImage(enemyId) {
  const webpPath = getEnemyImagePath(enemyId);
  const pngPath = getEnemyImagePathPng(enemyId);
  return preloadImage(webpPath).catch(() => preloadImage(pngPath));
}
