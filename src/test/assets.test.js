...
describe('Asset Pipeline - Enemy Art', () => {
  ...
  describe('getEnemyImagePath', () => {
    it('should return correct path for a simple enemy ID', () => {
      expect(getEnemyImagePath('cultist')).toBe('/images/enemies/cultist.webp');
    });

    it('should return correct path for an underscore enemy ID', () => {
      expect(getEnemyImagePath('louse_red')).toBe('/images/enemies/louse_red.webp');
    });

    it('should return correct path for a camelCase enemy ID', () => {
      expect(getEnemyImagePath('jawWorm')).toBe('/images/enemies/jawWorm.webp');
    });

    it('should return correct path for a boss enemy ID', () => {
      expect(getEnemyImagePath('slimeBoss')).toBe('/images/enemies/slimeBoss.webp');
    });
  });
  ...
  describe('Enemy ID naming conventions', () => {
    ...
    it('all enemy IDs should produce valid WebP file paths', () => {
      ALL_ENEMIES.forEach(enemy => {
        const path = getEnemyImagePath(enemy.id);
        expect(path).toMatch(/^\/images\/enemies\/[a-zA-Z][a-zA-Z0-9_]*\.webp$/);
      });
    });
    ...
    it('all enemies should have image paths that follow the naming convention {enemyId}.webp', () => {
      ALL_ENEMIES.forEach(enemy => {
        const path = getEnemyImagePath(enemy.id);
        const expectedPath = `/images/enemies/${enemy.id}.webp`;
